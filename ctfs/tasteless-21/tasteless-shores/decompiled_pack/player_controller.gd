...
onready  var placePivot = $placepivotas Position3D
...
func _ready():
	...
	($SpringArm / Cameraas Camera).make_current()
...
func _process(_delta):
	...
	if placeRayCast.enabled and placeRayCast.get_collision_point() != Vector3.ZERO:
...
func _physics_process(delta):
	...
	if player.health > 0 and not paused and Input.get_mouse_mode() == Input.MOUSE_MODE_CAPTURED:
		...
		if Input.is_action_pressed("jump") and player.is_on_floor() and not player.boat and not player.drowning:
			vel.y = jumpForce
